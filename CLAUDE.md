...
- `install.rs` — оркестрация: `sync_all` (ваниль→JRE→файлы) лежит в основе
  ...
  (fail-open). Удаление/правка мода ловится stat'ом → докачка. `verify_files`
  (кнопка «Проверить файлы») — принудительный полный хеш-скан (страховка от
  bit-rot). `sync_files`/`sync_manifest` — докачка файлов. Прогресс — единым трекером
