...
- **Загрузчик:** NeoForge **1.20.1 / 47.1.106** (`net.neoforged:forge:1.20.1-47.1.106`).
...
2. 🚧 Установка окружения и модпак (способ Б):
   ...
   - ✅ JRE-entry в манифесте (сборщик качает Temurin 17 под windows-x64 и
     linux-x64 → `dist/java/`).
...
5. ✅ Автообновление: дифф-обновление модпака (`sync_files` по SHA-256) +
   ...
   `TAURI_SIGNING_PRIVATE_KEY` задан. Релиз: бамп `version` → коммит → тег `vX.Y.Z`.
...
- `java.rs` — `ensure_java`: скачивание/проверка/распаковка JRE (zip для Windows,
  ...
  `macos-x64`/`macos-arm64`). Живой тест `jre_pipeline` (`#[ignore]`).
...
- `install.rs` — оркестрация: `sync_files`/`sync_manifest` (докачка файлов),
  ...
  эмитят прогресс `sync://progress` (подписи понятны игроку: «Устанавливаем
  Minecraft 1.20.1», «Устанавливаем Java», «Устанавливаем моды…» по `FileKind`).
...
- `settings.rs` — постоянные настройки лаунчера в `app_config_dir/settings.json`
  (главное — `install_dir`). Лаунчер «помнит» выбранную папку между запусками и
  после своего обновления → не предлагает переустановку игры заново.
...
- `lib.rs` — Tauri-команды: `default_install_dir`, `get_install_dir`/
  `set_install_dir` (запомненный путь), `is_game_installed`, `uninstall_game`,
  `validate_install_path`, `get_manifest`, `ensure_java`, `ensure_vanilla`,
  `sync_files`, `launch_game`, `play` (+ демо `greet`). `play` запоминает путь
  установки; `launch_game`/`play`/`uninstall_game` уносят блокирующую работу в
  `spawn_blocking`. Плагины: log/opener/dialog/updater/process.
...
- **Кнопка-папка** (📁, правый нижний угол) — `openInstallDir` через
  `tauri-plugin-opener` (`openPath`), открывает папку установки игры.
...
- **Выбор папки** — `pickInstallDir` через `tauri-plugin-dialog` (системный
  ...
  Выбранный путь сохраняется (`set_install_dir`), при старте читается
  `get_install_dir` → лаунчер помнит, где игра.
