...
- `install.rs` — оркестрация: `sync_all` (ваниль→JRE→файлы) лежит в основе
  ...
  т.е. и при установке, и при «Играть». `is_installed` — проверка наличия игры
  (JRE + client.jar) для подписи кнопки;
...
- `launch.rs` — `build_args`/`launch`: сливает ванильный + NeoForge version
  ...
  Ванильный `client.jar` (`versions/1.20.1/1.20.1.jar`) в classpath НЕ кладём:
