...
- **Источник модпака:** GitHub **Pages** `https://kingdom-rp.github.io/krp-mod`
  ...
  **Зеркало:** Yandex Object Storage (`config::MIRRORS`, path-style
  `https://storage.yandexcloud.net/kingdomrp`) как fallback, когда GitHub
  недоступен (блокировки из РФ). Зеркало повторяет раскладку `dist/` → ключ =
  `FileEntry.path`; CI krp-mod льёт `dist/` и на Pages, и на S3 (`aws s3 sync
  --delete`). Зеркалятся И наши файлы (Pages), И сторонние моды (их сборщик тоже
  кладёт в `dist/mods/`) — primary-url у каждого прежний, S3 общий fallback по
  path. Ваниль (Mojang CDN) НЕ зеркалится. Порядок: primary первым (экономит
  egress S3); как только зеркало выручило раз — в сессии оно первым (липкий
  `PREFER_MIRROR`, заблокированный игрок не ждёт таймаут GitHub на каждом файле).
...
- `config.rs` — константы: `MANIFEST_BASE_URL` (GitHub Pages kingdom-rp.github.io/krp-mod),
  ...
  `docs/manifest-signing.md`), `manifest_sig_url()`. `MIRRORS` — зеркала base-URL
  (Yandex S3) как fallback; `url_candidates(url, mirror_key)` строит список
  кандидатов primary→зеркало (зеркало = `{MIRROR}/{ключ}`, ключ = path в `dist/`;
  `mirror_key=None` → вывести из url снятием base-префикса, не-наш url = без
  зеркал). `set_prefer_mirror`/`PREFER_MIRROR` — липкое предпочтение зеркала на
  сессию после первого срабатывания.
...
- `manifest.rs` — структуры манифеста (`Manifest`, `FileEntry`, `JavaEntry`,
  ...
  `fetch_manifest` и подпись качаются через `get_text_mirrored` (primary→зеркало).
- `download.rs` — `download_to_file` (потоковый прогресс; качает в `<dest>.part`
  ...
  готового; перебирает `config::url_candidates` — primary, затем зеркала, успех с
  зеркала ставит `PREFER_MIRROR`), `sha256_file`, `ensure_file` (скип по совпадению
  хеша + проверка после загрузки → устойчивость к прерванной установке, докачка
  только битых файлов). Оба берут `mirror_key: Option<&str>` (для файлов манифеста
  = `FileEntry.path`; ваниль/Mojang = `None`).
...
- `vanilla.rs` — `ensure_vanilla`: резолв и загрузка ванильного MC с Mojang CDN
  ...
  `force`-параметр: `false` (запуск) — если есть маркер `versions/<id>/.krp_vanilla_ok`
  (пишется после успешной установки), весь скан ассетов (~2500 файлов/~300 МБ)
  пропускается (ассеты Mojang неизменны для версии); `true` («Проверить файлы») —
  полный SHA-1-скан. Убирает главный тормоз запуска (перехеш 300 МБ каждый старт).
  Живой тест `vanilla_resolver` (`#[ignore]`).
- `install.rs` — оркестрация: `sync_all` (ваниль→JRE→файлы) лежит в основе
  `install_only` (установка без запуска) и `play`. **`play` не ресинкает каждый
  раз:** `fetch_manifest_retry` (3 попытки, GitHub Pages бывает 404) → если
  манифест есть и `state_up_to_date` (версия == слепку И `size`+`mtime` всех
  клиентских файлов совпали, только `stat` — без хеширования) → быстрый запуск
  без ресинка (только prune+options+servers.dat); иначе полный `sync_all` +
  `write_state`. Манифест НЕдоступен, но игра стоит → **офлайн-запуск** на текущих
  файлах (профиль/injector из слепка `state.rs`) — не блокируем игрока из-за 404
  (fail-open). Удаление/правка мода ловится stat'ом → докачка. `sync_all`/
  `sync_manifest` берут `force`: на быстром пути (version-bump без правки файлов)
  файлы манифеста проверяются по `size`+`mtime` из слепка БЕЗ хеширования (не
  читаем ~200 МБ модов каждый старт), докачивается только несовпавшее (SHA-256
  после загрузки). `verify_files` (кнопка «Проверить файлы», `force=true`) —
  принудительный полный SHA-скан ванили+файлов (страховка от bit-rot).
...
- `state.rs` — локальный слепок установки (`<install>/.krp_state.json`): версия
  манифеста + neoforge_profile/authlib_injector + `size`+`mtime` каждого файла.
  Пишется после sync (`write_state`), читается в `play` для version-gate (быстрый
  запуск без ресинка) и офлайн-запуска. `mark(path)` — `stat` (size+mtime).
...
- `settings.rs` — постоянные настройки лаунчера в `app_config_dir/settings.json`
  ...
  переустановку заново. Настройки запуска: `max_memory_mb` (память, зажим в
  `config::MIN/MAX_MEMORY_MB`), `use_custom_jvm`+`custom_jvm_args` (свои JVM-аргументы).
  `settings::jvm_args(app)` → ведущие JVM-аргументы: кастомные (если включены и
  непустые) ИЛИ рекомендуемые (`-Xms/-Xmx` + `config::JVM_PERF_ARGS`). Команды
  `get_launch_settings`/`set_launch_settings`. UI — окно настроек (`SettingsModal`,
  открывается из бургер-меню ☰ справа снизу): слайдер памяти + radio
  «рекомендуемые / свои» (свои — с предупреждением для опытных).
...
- `launch.rs` — `build_args`/`launch`: сливает ванильный + NeoForge version
  ...
  JVM-аргументы первыми: `jvm_prefix` (из `settings::jvm_args` — рекомендуемые
  `-Xms/-Xmx`+G1GC либо кастомная строка), затем ваниль+NeoForge.
...
**Сторонние моды.** primary-url в манифесте = **внешний** CDN мода (не base_url):
лаунчер качает их напрямую в `mods/` и сверяет хеш; `prune_mods` их не трогает
(они в манифесте). Для **зеркала** (S3) сборщик дополнительно копирует каждый
скачанный jar из `modcache` в `dist/mods/` → `aws s3 sync` заливает их на S3;
лаунчер при недоступности CDN тянет по mirror-ключу = path (`mods/<jar>`). Т.е.
primary у мода прежний (внешний CDN), а S3 — общий fallback по path. Два источника
