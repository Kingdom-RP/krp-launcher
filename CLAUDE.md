...
- `java.rs` — `ensure_java`: скачивание/проверка/распаковка JRE (zip для Windows,
  ...
  (`JAVA_MAJOR=17`, `java_is_valid`/`parse_java_major`) — битую/чужую версию
...
- `install.rs` — оркестрация: `sync_all` (ваниль→JRE→файлы) лежит в основе
  ...
  («Устанавливаем Minecraft 1.20.1», «Устанавливаем Java», «…моды» по `FileKind`).
...
- `lib.rs` — Tauri-команды: `default_install_dir`, `get_install_dir`/
  ...
  «Kingdom RP»), `is_game_installed`, `uninstall_game`, `validate_install_path`,
  `get_manifest`, `ensure_java`, `ensure_vanilla`, `sync_files`, `install_game`
  (установка без запуска), `launch_game`, `play` (+ демо `greet`). `install_game`/
  `play` запоминают путь установки; `launch_game`/`play`/`uninstall_game` уносят
  блокирующую работу в `spawn_blocking`. В `run()` — panic-hook, пишущий паники
  лаунчера в лог. Плагины: log/opener/dialog/updater/process.
