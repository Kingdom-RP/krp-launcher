...
5. 🚧 Автообновление: дифф-обновление модов — готово (способ Б, `sync_files` по
   SHA-256). Автообновление самого лаунчера — код готов (Tauri Updater: плагины
   `updater`+`process`, баннер в UI, CI `release.yml` через `tauri-action`).
   Осталось разово: сгенерить ключ подписи → вставить pubkey в `tauri.conf.json`
   → секреты `TAURI_SIGNING_PRIVATE_KEY(_PASSWORD)` → тег `vX.Y.Z`.
...
- `tauri.conf.json`: `bundle.createUpdaterArtifacts: true`, `plugins.updater`
  (`pubkey` — ЗАМЕНИТЬ после генерации; `endpoints` → GitHub Releases
...
- `lib.rs` — Tauri-команды: `default_install_dir`, `validate_install_path`,
  ...
  `play` (+ демо `greet`). Плагины: log/opener/updater/process.
