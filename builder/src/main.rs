...
//! 5. (Опц.) Сторонние моды (`--modlist <toml>` / `--mods-release`): качает
//!    каждый в кэш, считает/сверяет SHA-256, пишет в манифест ВНЕШНИЙ url (primary
//!    = их CDN, НЕ base_url). Копию jar кладёт в `dist/mods/` для ЗЕРКАЛА (S3):
//!    лаунчер при недоступности CDN тянет по mirror-ключу = path.
//!
